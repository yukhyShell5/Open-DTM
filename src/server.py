    ...
    if available_workers:
        ...
        if worker_id in pending_worker_requests:
            ...
            if not future.done():
                ...
                logger.info(
                    f"Task {task_id} immediately assigned to worker {worker_id}"
                )
...
async def get_stats():
    stats = {
        ...
        "processing_tasks": sum(
            1 for t in tasks_db.values() if t["status"] == "processing"
        ),
        "completed_tasks": sum(
            1 for t in tasks_db.values() if t["status"] == "completed"
        ),
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "tasks": tasks, "workers": workers, "stats": stats},
    )
